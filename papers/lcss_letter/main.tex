...
\begin{figure}[t]
  ...
  \caption{Statement-dependency map and proof roadmap. Solid green =
  \provtag{} (proved, with the proof or sketch in this letter); grey = the
  sheaf construction (definitions); dashed red = the \conjtag{} stochastic
  steady-state object, deliberately shown \emph{outside} the proved chain ---
  it is the boundary discussed in Sec.~\ref{sec:notclaim}, not a dependency
  of the theorem.}
  \label{fig:thmmap}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Assumptions: role, how checked, and consequence when violated.}
\label{tab:assume}
\begin{tabular}{p{1.7cm}p{2.6cm}p{3.4cm}}
\toprule
assumption & mathematical role & checked by / violated \\
\midrule
taut cable, length $l$ & the trivialization $m(s)$ exists & broadside margin
$|\cos\sigma_i| > 0.1$ enforced in every draw; near broadside the reduced
plant leaves its validity domain (companion, \S VI) \\
common twist $\xi \ne 0$ & generates the transports $e^{\tau C_j}$ &
switch-off arm $\xi = 0$: holonomy $= 0.0$ exactly \\
persistent turn $\eta \ne 0$ & leading commutator $\propto \eta$ &
switch-off arm $\eta = 0$: machine zero ($<10^{-17}$) \\
uniform lag $\tau$ on the cycle & the frozen-generator BCH collection &
protocol class $\mathcal{P}$ asserts realized age $\equiv \tau$ per fuse;
jitter/drop arms are declared outside $\mathcal{P}$ \\
distinct shapes & $[C_i, C_j] \ne 0$ & coincident and, more generally,
...
slopes, whose entire support spans $3\times10^{-13}$ around 1.9993.
...
\begin{figure}[t]
  ...
  slopes: support width $3\times10^{-13}$.}
  \label{fig:remstats}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Theorem-to-experiment correspondence.}
\label{tab:corr}
\begin{tabular}{p{2.0cm}p{1.6cm}p{1.9cm}p{1.6cm}}
\toprule
claim & measurement & figure/table & outcome \\
\midrule
order 2 (Thm~\ref{thm:floor}) & log--log slope, 6 $\tau$ & Fig.~\ref{fig:e3a},
\ref{fig:ext}a & 1.999 / 2.000 both plants \\
coefficient 1 ($m{=}2$) & ratio at small $\tau$ & Fig.~\ref{fig:e3a},
\ref{fig:bound} & 1.0000 \\
null: $s_i{=}s_j$ (Cor.~\ref{cor:sym}) & switch-off arm & ladder table &
$<10^{-15}$ \\
null: $\eta{=}0$ & switch-off arm & ladder table & $<10^{-17}$ \\
null: $\xi{=}0$ & switch-off arm & ladder table & exactly $0.0$ \\
null: $\tau{=}0$ & explicit arm & Sec.~III-E(iii) & exactly $0.0$ \\
$O(\tau^3)$ uniform & 220-draw sup & Fig.~\ref{fig:remstats} & $\le 0.0133$ \\
level-set class & C15 search + torus scan & Fig.~\ref{fig:heatmap} & discrete;
exact protection \\
domain boundary & large-$\tau$ arm & Fig.~\ref{fig:domain} & 10\% at
$\tau{\approx}10$ \\
$D_{ss}$ (\conjtag) & companion E3b & Fig.~\ref{fig:forest} & falsified at
these scales \\
\bottomrule
\end{tabular}
\end{table}
...
running below. One movie, two behaviors, one changed variable: the
commutator.
...
per-formation fit, and the slope strip collapsing onto $1.9993$ with spread
$10^{-13}$. The order is structural; the geometry moves only the
coefficient.
